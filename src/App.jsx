...
import { MATURITIES, SPREADS } from './constants/maturities';
import './App.css';

function Dashboard() {
  const { yieldData, spreadData, anyLoading, loadingMessage, lastUpdated } = useYieldDataContext();
      ...
      <main className="dashboard">
        ...
        {anyData && (
          <>
            ...
            <div className="section-label">Yield Curve Spreads</div>
            <div className="chart-grid">
              {SPREADS.map((spread) => {
                const d = spreadData?.[spread.id];
                if (!d?.enrichedData) return null;
                return (
                  <BollingerChart
                    key={spread.id}
